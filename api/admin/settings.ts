 ...
 *
 * Body (PATCH):
 *   { default_session_turn_limit?: number, default_daily_turn_limit?: number }
 */
export default async function handler(req: Request): Promise<Response> {
  ...
  try {
    ...
    if (req.method === 'PATCH') {
      ...
      if (typeof body.default_daily_turn_limit === 'number') {
        updates.push({ key: 'default_daily_turn_limit', value_bigint: Math.trunc(body.default_daily_turn_limit) });
      }
    ...
    const { data, error } = await admin
      ...
      .in('key', ['default_session_turn_limit', 'default_daily_turn_limit']);
    ...
    return json(200, {
      data: {
        ...
        default_daily_turn_limit: map.default_daily_turn_limit ?? 200,
      },
    });
  } catch (e: any) {
